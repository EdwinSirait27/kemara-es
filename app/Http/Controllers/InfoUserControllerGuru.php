...
{
    $tombol = Tombol::where('url', 'DatakuGuru')->first();
    if (!$tombol) {
        return redirect()->route('dashboardGuru.index')->with('warning', 'Dataku tidak tersedia.');
    }
    ...
    if (Carbon::now()->between($start_date, $end_date)) {
        return view('laravel-examples/DatakuGuru', compact('user', 'roles'));
    }

    return redirect()->route('dashboardGuru.index')->with('warning', 'Dataku masih tertutup.');
}
