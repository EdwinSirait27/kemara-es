...
class InfoUserControllerKepalaSekolah extends Controller
{
    ...
    return view('laravel-examples/user-profileKepalaSekolah', compact('user', 'roles'));
    }
    ...
    {
        $tombol = Tombol::where('url', 'DatakuKepalaSekolah')->first();
        if (!$tombol) {
            return redirect()->route('dashboardKepalaSekolah.index')->with('warning', 'Dataku tidak tersedia.');
        }
        ...
        if (Carbon::now()->between($start_date, $end_date)) {
            return view('laravel-examples/DatakuKepalaSekolag', compact('user', 'roles'));
        }
    
        return redirect()->route('dashboardKepalaSekolah.index')->with('warning', 'Dataku masih tertutup.');
    }
