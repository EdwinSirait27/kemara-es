...
@section('content')
@section('title', 'Kemara-ES | Edit Informasi')
...
<div>
    <div class="container-fluid">
        ...
        <form action="{{ route('Informasi.update', $hashedId) }}" method="POST"enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="container-fluid py-4">
                <div class="card">
                    <div class="card-header pb-0 px-3">
                        <h6 class="mb-0">{{ __('Edit Informasi') }}</h6>
                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div>
                                        <input type="text" class="form-control" id="header" name="header"
                                            value="{{ old('header', $informasippdb->header) }}" required maxlength="255">
                                ...
                                <div class="form-group">
                                    ...
                                    <div>
                                        <textarea 
                                            ...
                                            required
                                            style="resize: both; overflow: auto;">{{ e($informasippdb->body ?? '') }}</textarea>
                                        @error('body')
                                    ...
                                    <div class="@error('status') border border-danger rounded-3 @enderror">
                                        <select class="form-control" name="status" id="status" required>
                                            <option value="" disabled
                                                {{ old('status', $informasippdb->status ?? '') == '' ? 'selected' : '' }}>
                                                Pilih Status</option>
                                            <option value="Aktif"
                                                {{ old('status', $informasippdb->status ?? '') == 'Aktif' ? 'selected' : '' }}>
                                                Aktif</option>
                                            <option value="Tidak Aktif"
                                                {{ old('status', $informasippdb->status ?? '') == 'Tidak Aktif' ? 'selected' : '' }}>
                                                Tidak Aktif</option>
                                        </select>
                                        @error('status')
                    ...
                    <div class="d-flex justify-content-end">
                        ...
                        <a href="{{ route('Informasi.index') }}" class="btn btn-secondary mt-4 mb-4">
                            {{ __('Cancel') }}
                        </a>
                    </div>
